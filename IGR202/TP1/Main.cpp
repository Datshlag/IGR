...
static const string DEFAULT_MESH_FILE ("models/HANZO.off");
...
static Mesh mesh;
static LightSource lightSource = LightSource(Vec3<float>(0,0,-1),Vec3<float>(1.0,0.0,1.0));
GLProgram * glProgram;
...
void updatePerVertexColorResponse () {
    ...
    for (unsigned int i = 0; i < colorResponses.size (); i++){
				...
				Vec3<float> omegaI = mesh.positions()[i] - lightSource.getPos();
				float scProduct = dot(omegaI, mesh.normals()[i]);
				float attenuation = 1/(1*d+d*d);
        colorResponses[i]=attenuation*lightSource.getColor()*scProduct;
		}
}
...
void key (unsigned char keyPressed, int x, int y) {
    switch (keyPressed) {
	    case 'f':
	        ...
	        break;
			case 'z':
				lightSource.addX(1.0);
				break;
			case 's':
				lightSource.addX(-1.0);
				break;
			case 'q':
				lightSource.addY(1.0);
				break;
			case 'd':
				lightSource.addY(-1.0);
				break;
			case '+':
				lightSource.addLum(0.01);
				break;
			case '-':
				lightSource.addLum(-0.01);
				break;
